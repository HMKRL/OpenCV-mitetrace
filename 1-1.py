...
def detect(filename, real_width):
    ...
    for index, point in enumerate(center_rec):
        ...
        if dis < 80:
            ...
            ratio = 510 * index / len(center_rec)
            green = ratio if ratio <= 255 else 255
            red = 255 if ratio <= 255 else ratio - 255
            cv2.circle(base_frame, point, 5, (0, green, red, 255), -1)
            if index > 0:
                cv2.line(base_frame, center_rec[index], center_rec[index - 1], (0, green, red, 255), 2)
