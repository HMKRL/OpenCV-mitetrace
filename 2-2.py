...
def detect(filename, real_width):
    ...
    for index, point in enumerate(center_rec):
        ...
        if dis < 80:
            ...
            ratio = 255 * index / len(center_rec)
            cv2.circle(base_frame, point, 5, (0, 255, 255 - ratio, 255), -1)
            if index > 0:
                cv2.line(base_frame, center_rec[index], center_rec[index - 1], (0, 255, 255 - ratio, 255), 2)
